...
export default function CreateGettingOutTheDoorChecklistPage() {
  const [favoriteThing, setFavoriteThing] = useState("");
  const [routineItems, setRoutineItems] = useState<string[]>(["get dressed", "brush teeth", "eat breakfast", "put on shoes"]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const prompt = `Create a personalized morning/getting out the door checklist for a child whose favorite thing is "${favoriteThing}". The routine includes: ${validItems.join(", ")}. Include praise phrases and positive reinforcement tips.`;
      
      const res = await fetch("/api/content/generate", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          category: "getting_out_the_door",
          type: "checklist",
          prompt,
          favoriteThing: favoriteThing,
          routineItems: validItems,
        }),
      });
  ...
  return (
    <>
      <header className="sticky top-0 z-10 flex items-center justify-between bg-background-light p-4 dark:bg-background-dark">
        <Link href="/library/getting-out-the-door" className="flex h-10 w-10 items-center justify-center rounded-full text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700">
      ...
      <main className="p-4 pb-20">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            <label className="mb-2 block text-sm font-medium">What's your morning routine?</label>
            <p className="mb-3 text-xs text-[#617c89] dark:text-[#a0b3bd]">
              Add items like: get dressed, brush teeth, eat breakfast, put on shoes
            </p>
