...
export default function CreateChoresChecklistPage() {
  const [favoriteThing, setFavoriteThing] = useState("");
  const [routineItems, setRoutineItems] = useState<string[]>(["put away toys", "set the table", "feed the pet"]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (validItems.length === 0) {
      setError("Please add at least one chore item");
      setLoading(false);
      return;
    }

    try {
      const prompt = `Create a personalized chore checklist for a child whose favorite thing is "${favoriteThing}". The chores include: ${validItems.join(", ")}. Include psychological background on compliance and positive reinforcement tips.`;
  ...
  return (
    <>
      <header className="sticky top-0 z-10 flex items-center justify-between bg-background-light p-4 dark:bg-background-dark">
        <Link href="/library/chores" className="flex h-10 w-10 items-center justify-center rounded-full text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700">
      ...
      <main className="p-4 pb-20">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            <label className="mb-2 block text-sm font-medium">What chores would you like your child to do?</label>
            <p className="mb-3 text-xs text-[#617c89] dark:text-[#a0b3bd]">
              Add items like: put away toys, set the table, feed the pet
            </p>
